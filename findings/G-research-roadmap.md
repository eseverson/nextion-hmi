...
|---|---------|-------------|--------|
| H1 | Page-blob CRC algorithm (zlib.crc32 doesn't match) | Brute-force every CRC32 polynomial × seed × reflect against baseline 0.pa and 04 0.pa (known 4-byte payload diff) | `[!]` 384 standard CRC-32 variants × 9 payload ranges all rejected ([crack_page_crc.py](../scripts/crack_page_crc.py)). Algorithm is custom (not a textbook CRC). Could be: a CRC over name+payload, a non-CRC hash, or includes external data. Needs deeper reverse engineering. |
...
| # | Unknown | How to crack | Status |
|---|---------|-------------|--------|
| T1 | F-series H2 XOR key (TFTTool placeholder = 0; H2 is clearly encrypted) | Read TFTTool source ([K](K-tfttool-source-h2-attack.md)); derive key from known-plaintext addresses at H2+0x08, 0x18 | `[~]` Cipher is NOT 4-byte-repeating XOR (verified with two known plaintexts giving inconsistent keys). Could be longer-key XOR, OTP-style position-dependent pad, or different cipher. Diffusion map from [J](J-h2-transform-analysis.md) still holds. Partial pad recovered at H2+0x08..0x0b = `de 95 4c 03`. Next attacks: longer-key brute force; firmware blob reverse engineering; targeted single-resource experiments. |
