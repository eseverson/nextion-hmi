...
| # | Unknown | How to crack | Status |
|---|---------|-------------|--------|
| T1 | F-series H2 XOR key (TFTTool placeholder = 0; H2 is clearly encrypted) | Brute-force the transform using {04,05,06,08,09,10,11} as multi-known-plaintext: each maps a known file_size delta to a known H2 delta | `[~]` Diffusion map decoded per [J](J-h2-transform-analysis.md): each H1 file_size byte affects 5 H2 byte positions (byte 0 → H2 0x00,0x04,0x0c,0x18,0x1c; byte 1 → H2 0x01,0x1d,0x34,0x38,0x3a). Not a simple XOR. Need finer-grained experiments (single-byte fs deltas) to fully reverse. |
