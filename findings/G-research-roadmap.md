...
|---|---------|-------------|--------|
| H1 | Page-blob CRC algorithm (zlib.crc32 doesn't match) | `04_red_val_deadbeef` produces a known 4-byte payload diff; try every CRC32 polynomial+seed against pre/post payloads | `[ ]` |
...
| H12 | Where Text `txt` strings live (length-prefixed? null-terminated?) | `05_text_qqqqqqqq` — known plaintext | `[ ]` |
| H13 | New page id allocation (lowest-free vs len(pages) vs other) | `11_add_page` | `[ ]` |
